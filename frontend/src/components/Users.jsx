        ...
        <div className="inline-block min-w-full py-2 sm:px-10 lg:px-8">
          <div className="overflow-hidden">
            <table className="min-w-full text-left text-sm font-light mt-10">
              <tbody>
                {(isLoading) ?
                  <UserSkeleton count={8}/> : users.length === 0 && filter.length !== 0 ? <NoUserFound /> : users.map(user => <User key={user._id} id={user._id} fname={user.firstname} lname={user.lastname} /> || <UserSkeleton />)
                }
              </tbody>
            </table>
          </div>
